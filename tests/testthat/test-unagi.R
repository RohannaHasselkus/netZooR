...
test_that("[UNAGI] SignificantBreadthFirstSearchU() function yields expected results", {
  ...
  fullNetworks$sample1 <- fullNetwork1$score - 0.00005
  fullNetworks$sample2 <- fullNetwork1$score + 0.00005
  fullNetworks <- bindWithReversed(fullNetworks)
  ...
  result1 <- SignificantBreadthFirstSearchU(networks = subnetwork,
                                            startingNodes = c("geneA", "geneB", "geneC"),
                                            nodesToExclude = c())
  expect_setequal(rownames(result1), c("gene2__geneA", "gene2__geneB", "gene3__geneA", "gene4__geneC"))
  
  # Ensure that, when starting from genes gene2, gene3, and gene4 and removing genes A, B, and C, we obtain the correct values.
  result2 <- SignificantBreadthFirstSearchU(networks = subnetwork,
