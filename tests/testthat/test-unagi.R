...
test_that("[UNAGI] BreadthFirstSearchU() function yields expected results", {
  ...
  expect_error(BreadthFirstSearchU(networks = subnetwork,
  ...
  result1 <- BreadthFirstSearchU(networks = startingNetwork,
                                            startingNodes = c("geneA", "geneB", "geneC"),
                                            nodesToExclude = c())
  expect_setequal(rownames(result1), setdiff(rownames(startingNetwork), c("gene3__geneD", "gene4__geneD")))
  ...
  expect_setequal(rownames(result2), c("gene3__geneD", "gene4__geneD"))
...
test_that("[UNAGI] FindConnectionsForAllHopCountsU() function yields expected results", {
  ...
  if (is.data.frame(result) || is.matrix(result)) {
    expect_equal(rownames(result), 
                 c("gene3__geneD", "gene4__geneD", "geneA__gene3", "geneC__gene4"))
  } else {
    stop("Error: FindConnectionsForAllHopCountsU did not return a data frame or matrix.")
  }
})
...
test_that("[UNAGI] RunUNAGI() function yields expected results",{
  ...
  # Check errors.
  expect_error(RunUNAGI(geneSet = 4, networks = list(), hopConstraint = 5),
  ...
  expect_error(RunUNAGI(geneSet = "g1", networks = 67, hopConstraint = 5),
               paste("Wrong input type! geneSet must be a character vector. networks must be a list.",
                     "alpha and hopConstraint must be scalar numeric values."))
  expect_error(RunUNAGI(geneSet = "g1", networks = list(), hopConstraint = 5),
               paste("Wrong input type! geneSet must be a character vector. networks must be a list.",
                     "alpha and hopConstraint must be scalar numeric values."))
  expect_error(RunUNAGI(geneSet = "g1", networks = list(), hopConstraint = "hi"),
               paste("Wrong input type! geneSet must be a character vector. networks must be a list.",
                     "alpha and hopConstraint must be scalar numeric values."))
  expect_error(RunUNAGI(geneSet = "g1", networks = list(1,2,3), hopConstraint = 5),
  ...
  expect_error(RunUNAGI(geneSet = "g1", networks = list(data.frame(source = NA, target = NA, score = NA)), 
                           alpha = -1, hopConstraint = 5))
  expect_error(RunUNAGI(geneSet = "g1", networks = list(data.frame(source = NA, target = NA, score = NA)), 
                           alpha = 0, hopConstraint = 5))
  expect_error(RunUNAGI(geneSet = "g1", networks = list(data.frame(source = NA, target = NA, score = NA)), 
                           alpha = 1.2, hopConstraint = 5))
  expect_error(RunUNAGI(geneSet = "g1", networks = list(data.frame(source = NA, target = NA, score = NA)), 
                           alpha = 1, hopConstraint = -4), "hopConstraint must be an even number of at least 2.")
  expect_error(RunUNAGI(geneSet = "g1", networks = list(data.frame(source = NA, target = NA, score = NA)), 
                           alpha = 1, hopConstraint = 7), "hopConstraint must be an even number of at least 2.")
  expect_error(RunUNAGI(geneSet = "g1", networks = list(data.frame(source = NA, target = NA, score = NA)), 
                           alpha = 1, hopConstraint = 4))
})